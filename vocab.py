...
Usage:
    vocab.py --notes_file=<file> --labels_file=<file> [options] VOCAB_FILE
...
from utils import read_corpus, pad_sents, get_all_labels
...
class Vocab(object):
    ...
    """
    def __init__(self, src_vocab: VocabEntry, num_labels):
        """ Init Vocab.
        @param src_vocab (VocabEntry): VocabEntry for source language
        @param tgt_vocab (VocabEntry): VocabEntry for target language
        """
        self.notes_ = src_vocab
        self.num_labels = num_labels

    @staticmethod
    def build(notes, vocab_size, freq_cutoff, num_labels) -> 'Vocab':
        """ Build Vocabulary.
        ...
        notes = VocabEntry.from_corpus(notes, vocab_size, freq_cutoff)

        return Vocab(notes, num_labels)

    def save(self, file_path):
        """ Save Vocab to file as JSON dump.
        @param file_path (str): file path to vocab file
        """
        json.dump(dict(notes_word2id=self.notes_.word2id, num_labels=self.num_labels), open(file_path, 'w'), indent=2)
    ...
    def load(file_path):
        ...
        notes_word2id = entry['notes_word2id']
        num_labels = int(entry['num_labels'])

        return Vocab(VocabEntry(notes_word2id), num_labels)
...
## Command
# python vocab.py --notes_file='./data/tiny_data_text.csv' vocab.json
if __name__ == '__main__':
    args = docopt(__doc__)

    print("parsing notes file: ")
    train_data_notes = read_corpus(args['--notes_file'], 
                                        column='text',
