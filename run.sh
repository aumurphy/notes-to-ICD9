...
# 	CUDA_VISIBLE_DEVICES=0 python run.py train --notes_file=./data/small/train_text.csv --labels_file=./data/small/train_labels.csv --dev-notes=./data/small/dev_text.csv --dev-labels=./data/small/dev_labels.csv --vocab=vocab.json --cuda --sent_max_length=1000 --remove_stopwords --max-epoch=20 --save-to=./node_models_2bilstm_500.bin --valid-niter=30 
	CUDA_VISIBLE_DEVICES=0 python run.py train --notes_file=./data/tiny/train_text.csv --labels_file=./data/tiny/train_labels_rolled_up.csv --dev-notes=./data/tiny/dev_text.csv --dev-labels=./data/tiny/dev_labels_rolled_up.csv --vocab=vocab.json --cuda --sent_max_length=1000 --remove_stopwords --max-epoch=20 --save-to=./node_models_2bilstm_500_full_ru.bin --valid-niter=300 
elif [ "$1" = "test" ]; then
