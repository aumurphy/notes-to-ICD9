...
if [ "$1" = "train" ]; then
	CUDA_VISIBLE_DEVICES=0 python run.py train --notes_file=./data/tiny/train_text.csv --labels_file=./data/tiny/train_labels.csv --dev-notes=./data/tiny/dev_text.csv --dev-labels=./data/tiny/dev_labels.csv --vocab=vocab.json --cuda --sent_max_length=1000 --remove_stopwords --max-epoch=30 --save-to=./node_models_2bilstm_500.bin --valid-niter=30
elif [ "$1" = "test" ]; then
	...
	python run.py train --notes_file=./data/tiny/train_text.csv --labels_file=./data/tiny/train_labels.csv --sent_max_length=1000 --remove_stopwords --dev-notes=./data/tiny/dev_text.csv --dev-labels=./data/tiny/dev_labels.csv --vocab=vocab.json --valid-niter=30 --save-to=./node_models_2bilstm_500.bin --max-epoch=30
elif [ "$1" = "train_local_small" ]; then
	python run.py train --notes_file=./data/small/train_text.csv --labels_file=./data/small/train_labels.csv --sent_max_length=1000 --remove_stopwords --dev-notes=./data/small/dev_text.csv --dev-labels=./data/small/dev_labels.csv --vocab=vocab.json --save-to=./node_models_2bilstm_500.bin --max-epoch=30 --valid-niter=30
