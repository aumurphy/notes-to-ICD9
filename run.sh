...
if [ "$1" = "train" ]; then
	...
	python run.py train --notes_file=./data/tiny/train_text.csv --labels_file=./data/tiny/train_labels.csv --sent_max_length=500 --remove_stopwords --dev-notes=./data/tiny/dev_text.csv --dev-labels=./data/tiny/dev_labels.csv --vocab=vocab.json --valid-niter=30
elif [ "$1" = "train_local_small" ]; then
	python run.py train --notes_file=./data/small/train_text.csv --labels_file=./data/small/train_labels.csv --sent_max_length=500 --remove_stopwords --dev-notes=./data/small/dev_text.csv --dev-labels=./data/small/dev_labels.csv --vocab=vocab.json
